...
router.get('/:interview_id', function(req, res, next) {
    try {
        var ideaId = req.param('interview_id');
        ...
        req.getConnection(function(err, conn) {
            if (err) {
                ...
                conn.query('SELECT id, title, notes, create_user, create_datetime, update_user, update_datetime FROM interviews WHERE id = ?', ideaId, function(err, rows, fields) {
                    if (err) {
                        console.error('SQL Error: ', err);
                        return next(err);
                    }
